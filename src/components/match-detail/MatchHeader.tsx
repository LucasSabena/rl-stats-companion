...
export const MatchHeader = memo(function MatchHeader({ match }: MatchHeaderProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-border-subtle bg-bg-secondary p-6 shadow-level-2">
      {/* Subtle gradient background */}
      <div className="absolute inset-0 bg-gradient-to-r from-[var(--color-team-blue-bg)] via-transparent to-[var(--color-team-orange-bg)] opacity-30" />
      ...
      <div className="relative">
        ...
        <div className="mt-8 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <MetaItem label="Arena" value={match.arena ?? "---"} />
        </div>
      </div>
    </div>
  );
...
function MetaItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-bg-tertiary/80 p-3 text-center">
